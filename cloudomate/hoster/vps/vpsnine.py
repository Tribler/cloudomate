...
class VPSNine(SolusvmHoster):
    ...
    def __init__(self, settings):
        super(VPSNine, self).__init__(settings)
    ...
    @staticmethod
    def get_metadata():
        return 'vpsnine', 'https://linevast.de/'
    ...
    def get_options(cls):
        ...
        browser.open("https://www.vpsnine.com/hosting/index.php")
    ...
    def _parse_openvz_hosting(cls, page):
        options = page.find_all('div', {'class': 'home-plan-box'})
        for idx, option in enumerate(options, start=1):
            name=option.find('h1').text.strip()
            if name == "Custom":
                continue
            list_elements = filter(None, option.find('p').text.replace('\t', '').split('\n'))
            memory=list_elements[0].split(' ')[0].split('+')[0]
            if 'MB' in memory:
                memory=float(memory.split('MB')[0])/1024
            else:
                memory=memory.split('GB')[0]
            # price_eur = float(option.find('div', {'class', 'price'}).span.text[1:])
            # c = CurrencyConverter()
            price_usd = float(option.find('div', {'class': 'home-plan-price'}).find('h1').text)
            yield VpsOption(
                name,
                storage=list_elements[2].split(' ')[0].split('GB')[0],
                cores=list_elements[1].split(' ')[0].split('x')[0],
                memory=memory,
                bandwidth=list_elements[3].split(' ')[0].split('+')[0],
                connection=1,
                price=price_usd,
                purchase_url='https://www.vpsnine.com' + option.find('a', {'title': 'order now'})['href'],
            )
